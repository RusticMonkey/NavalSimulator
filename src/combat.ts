import { CombatAlgorithms } from './combatinterfaces';
import { Dice } from './dice';
import { Gun } from './gun';
import { Ship } from './ship';

export class Combat implements CombatAlgorithms {
    fireGuns(sourceShip: Ship, targetShip: Ship, range: number, dicePool: Dice) : string[] {
        const combatLog = new Array<string>()
        sourceShip.shipClass.guns.forEach(gun => {
            if (this.checkRange(gun, range)) {
                ...
                let armourToUse = targetShip.shipClass.armourScheme.main
                if (plungingFire){
                    armourToUse = targetShip.shipClass.armourScheme.plunging
                }
                ...
                const damage = this.calculateDamage(penetrations, gun.calibre, targetShip.shipClass.scale, dicePool, combatLog)
                //todo - crits
    ...
    private resolveHits(gun: Gun, targetShip: Ship, targetArmour: Number, combatLog: string[], dicePool: Dice, isPlunging: boolean): {hits: number, crits: number} {
        let numCrits = 0
        const shots = Math.ceil((gun.barrels * gun.rateOfFire) / 3)
        return this.resolveSingleHit(shots, targetShip.shipClass.scale, dicePool, combatLog, isPlunging)
    }
